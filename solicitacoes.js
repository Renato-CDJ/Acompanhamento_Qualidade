...
class SolicitacoesSystem {
  ...
  setupEventListeners() {
    document.getElementById("addSolicitacaoForm").addEventListener("submit", (e) => this.addSolicitacao(e))

    document.getElementById("refreshSolicitacoesBtn").addEventListener("click", () => this.loadData())

    document.getElementById("searchSolicitacoes").addEventListener("input", (e) => this.filterTable(e.target.value))

    document.getElementById("filterSolicitacoesStatus").addEventListener("change", () => this.renderTable())
    ...
    document.getElementById("backToSolicitacoesMainBtn").addEventListener("click", () => {
  ...
  async addSolicitacao(e) {
    ...
    const solicitacao = {
      ...
      operadores: document
        .getElementById("solicitacaoOperador")
        .value.split(",")
        .map((op) => op.trim())
  ...
  updateStats() {
    ...
    document.getElementById("pendentesSolicitacoes").textContent = this.data.filter(
      (s) => s.status === "Pendente",
    ).length
    document.getElementById("concluidasSolicitacoes").textContent = this.data.filter(
      (s) => s.status === "Aplicado",
    ).length
  }
