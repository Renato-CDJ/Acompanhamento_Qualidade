...
class TrainingSystem {
  ...
  setupEventListeners() {
    ...
    document
      ...
      ?.addEventListener("change", () => this.renderTrainingStatusTable())

    document
      .getElementById("searchTrainingStatus")
      ?.addEventListener("input", (e) => this.renderTrainingStatusTable(e.target.value))
    document
      .getElementById("searchTrainingStatusTable")
      ?.addEventListener("input", (e) => this.renderTrainingStatusTable(e.target.value))
  ...
  showMainOverview(tabName = "treinamento") {
    ...
    allSections.forEach((section) => {
      if (
        !section.id.includes("mainOverviewSection") &&
  ...
  async addDesligamento(e) {
    ...
    const desligamento = {
      ...
      diasEmpresa: Number.parseInt(document.getElementById("desligDias").value, 10),
  ...
  renderTable(type) {
    ...
    data.sort((a, b) => {
      const nameA = (a.colaborador || a.nome || a.operador || "").toLowerCase()
      const nameB = (b.colaborador || b.nome || b.operador || "").toLowerCase()
    ...
    try {
      if (type === "training") {
        ...
        data = {
          ...
          diasEmpresa: Number.parseInt(document.getElementById("editDias").value, 10),
  ...
  getFilteredData(type) {
    ...
    if (this.globalMonthFilter) {
      ...
      data = data.filter((item) => {
        ...
        const dataCampo = item.dataAdicionado || item.dataTreinamento || item.dataDesligamento || item.data
  ...
  async handleExcelImport(e, type) {
    ...
    reader.onload = async (evt) => {
      ...
      for (const row of jsonData) {
        ...
        const dataExcel = row.data || row.Data || ""
        let dataAdicionado = ""
...
window.trainingStatus = async (docId, newStatus) => {
  try {
