...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (quadroForm) {
    quadroForm.addEventListener("submit", async (e) => {
      ...
      const total = Number.parseInt(document.getElementById("quadroTotal").value, 10)
      const ferias = Number.parseInt(document.getElementById("quadroFerias").value, 10)
      const desaparecidos = Number.parseInt(document.getElementById("quadroDesaparecidos").value, 10)
      const afastamento = Number.parseInt(document.getElementById("quadroAfastamento").value, 10)
      const inss = Number.parseInt(document.getElementById("quadroINSS").value, 10)
  ...
  if (editForm) {
    editForm.addEventListener("submit", async (e) => {
      ...
      const novoTotal = Number.parseInt(document.getElementById("editTotal").value, 10)
      const novoFerias = Number.parseInt(document.getElementById("editFerias").value, 10)
      const novoDesaparecidos = Number.parseInt(document.getElementById("editDesaparecidos").value, 10)
      const novoAfastamento = Number.parseInt(document.getElementById("editAfastamento").value, 10)
      const novoInss = Number.parseInt(document.getElementById("editInss").value, 10)
...
function carregarQuadros() {
  ...
  onSnapshot(query(collection(db, "quadro"), where("tipo", "==", quadroSelecionado)), (snapshot) => {
...
function renderCharts() {
  ...
  const totalManha =
    quadros
      .filter((q) => q.turno === "Manhã")
      .map((q) => q.ativos)
      .pop() || 0
  const totalTarde =
    quadros
      .filter((q) => q.turno === "Tarde")
      .map((q) => q.ativos)
      .pop() || 0
